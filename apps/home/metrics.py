...
def metricas_produtos():
    total_preco_custo = Produto.objects.aggregate(
        total=Sum(F('preco_custo') * F('quantidade')))['total'] or 0
    total_preco_venda = Produto.objects.aggregate(
        total=Sum(F('preco_venda') * F('quantidade')))['total'] or 0
...
def metricas_vendas():
    ...
    total_valor_vendas = vendas.aggregate(
        total_valor_vendas=Sum('quantidade') * F('produto__preco_venda'))['total_valor_vendas'] or 0
    total_valor_custo = vendas.aggregate(
        total_valor_custo=Sum('quantidade') * F('produto__preco_custo'))['total_valor_custo'] or 0
...
def metricas_saidas():
    vendas = Produto.objects.filter(movimentacao__tipo="Saída")
